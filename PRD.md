...
**Propósito:** Fornecer uma interface simples para que usuários humanos consultem a situação cadastral e os dados de Pessoas Jurídicas (PJ) diretamente no WebService da SEFAZ (focado na UF de MG).
...
- Integração de Backend (API) via monorepo com o WebService SOAP "ConsultaCadastro" da SEFAZ/MG.
...
3. **Integração SEFAZ (SOAP):** A API deve envelopar os dados no formato SOAP exigido pela SEFAZ/MG e anexar o Certificado Digital configurado.
...
- **Como** um humano querendo verificar uma empresa, **eu quero** digitar o CNPJ na tela, **para que** eu veja imediatamente o status cadastral e nome da empresa na SEFAZ-MG.
